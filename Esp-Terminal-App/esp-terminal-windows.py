...
from terminal import TerminalPage  # Import from your terminal.py
...
class SerialApp:
    ...
    def refresh_ports(self):
        ...
        print("Refreshed ports:", ports)

    def get_serial_ports(self):
        ports = serial.tools.list_ports.comports()
        ports_list = [port.device for port in ports if "COM" in port.device]
        print("Available ports:", ports_list)
        return ports_list
    ...
    def connect_serial(self):
        ...
        if port:
            try:
                ...
                self.serial_port = serial.Serial(port, 115200, timeout=1, rtscts=False, dsrdtr=False)
                print(f"Connected to {port}")
                time.sleep(1)  # Wait for potential boot log
                self.serial_port.reset_input_buffer()  # Clear boot log
                ...
                while time.time() < timeout:
                    if self.serial_port.in_waiting:
                        ...
                        print(f"Partial response: {response.decode(errors='ignore').strip()}")
                    time.sleep(0.1)

                decoded = response.decode(errors='ignore').strip()
                print(f"Raw response: {response!r}")
                print(f"Decoded response: {decoded}")
                print(f"Checking if 'ESP32' is in '{decoded}'")
                if "ESP32" in decoded:
                    print("Valid response received, proceeding to TerminalPage")
                    self.page1.pack_forget()
                    try:
                        self.page2 = TerminalPage(self.root, send_command_fn=self.send_uart_command)
                        self.page2.pack(fill="both", expand=True)
                        self.root.update()  # Force GUI update
                        print("TerminalPage loaded successfully")
                    except Exception as e:
                        print(f"Error loading TerminalPage: {e}")
                        if self.serial_port and self.serial_port.is_open:
                            self.serial_port.close()
                else:
                    print("No valid response from ESP32")
                    if self.serial_port and self.serial_port.is_open:
                        self.serial_port.close()
            except Exception as e:
